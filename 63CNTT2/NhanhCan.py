    ...
    while len(OPEN) > 0:        #OPEN <> 0
        ...
        for neighbor in range(len(adj)):
            if adj[curr][neighbor] !=0:
                #n khong thuoc ve OPEN va CLOSE
                if f[curr] < fmin and check_OPEN_CLOSE(OPEN,neighbor)==False and check_OPEN_CLOSE(CLOSE,neighbor)==False:
                    Tn.append(neighbor)
                    g[neighbor] = g[curr] + adj[curr][neighbor]
                    f[neighbor] = g[neighbor] + h[neighbor][1]
                    Parent[neighbor] = curr
                #n: thuoc OPEN(khong quan tam den CLOSE)
                if f[curr] < fmin and check_OPEN_CLOSE(OPEN,neighbor)==True:
                ...
                #n khong thuoc ve OPEN nhung thuoc ve CLOSE
                if f[curr] < fmin and check_OPEN_CLOSE(OPEN,neighbor)==False and check_OPEN_CLOSE(CLOSE,neighbor)==False:
                    Tn.append(neighbor)
                    g[neighbor] = g[curr] + adj[curr][neighbor]
                    f[neighbor] = g[neighbor] + h[neighbor][1]
        ...
        #Khong sap xep OPEN tang dan theo f
